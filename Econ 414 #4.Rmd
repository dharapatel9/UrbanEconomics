...
The equilibrium price is now 70. 
...
The change in the housing stock is 40. The total stock of housing is 60. 
...
The housing price would be 30. No, because this was the initial equilibrium price $p_{e}$. 
...
The income-grant program has a high utility level - households have the autonomy to choose the optimal quantities of q and c. However, the slum reduction effect is higher under a proportional rent subsidy. 

The public housing program yields the most slum reduction effect while generating the smallest margin of increase in utility level. 

I would advise the mayor to go with the public housing effect program. I think it's better in the long term to curb the negative externality of the slums rather than allow a targeted group to rise while keeping the slums down. 
...
The optimal level for $A$ is 30 and the optimal level for $C$ is 10.  
...
The election will go for $A$. Although, the optimal level is high, with one allocation you serve the same amount of people for about four allocations of $C$. 
...
The consumer surplus for population A is 450 and the consumer surplus for population C is 50. 
...
The consumer-voter surplus is gonna be 500. Yes, because this the optimal neighborhood. 
...
The new optimal level of policing is 30. 
...
The new consumer-voter C surplus is 200. Yes, they want to move. 
...
```{r,include=T}
library(tidycensus)
var<-load_variables("acs5", year=2018)
#View(var)
library(tmap)
tmap_mode("plot")
tm_shape(ghetto)+
tm_borders(lwd = 2, col = "white", alpha = .4)+
tm_polygons("blackE", breaks=c(0,.2,.4,.6,.8,1), palette = "OrRd", title="Population of Black in Chicago and Surrounding Counties")+  tm_legend(legend.title.size = 1,legend.text.size = .7, 
...
```{r, include=T}
tot_white<-sum(ghetto$whiteE)
tot_black<-sum(ghetto$blackE)

ghetto$X<-ghetto$blackE/tot_black
ghetto$Y<-ghetto$whiteE/tot_white
DI<-.5*sum(abs(ghetto$X-ghetto$Y))
DI*100
```
